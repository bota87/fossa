...
void feedmefiat()
{
  ...
  tft.setTextSize(2);
  tft.println(fossaT);
  String s = feedT + " " + String(charge) + chargeT;
  tft.setCursor((TFT_WIDTH - textWidth(s, 2)) / 2, 280);
  tft.setTextSize(2);
  tft.println(s);
}
void feedmefiatloop()
...
void qrShowCodeLNURL(String message)
{
  ...
  message = "Leggi il QR code e tocca";
  tft.setCursor((TFT_WIDTH - textWidth(message, 2)) / 2, 275);
  tft.println(message);

  message = "lo schermo quando hai finito";
  tft.setCursor((TFT_WIDTH - textWidth(message, 2)) / 2, 295);
  tft.println(message);
